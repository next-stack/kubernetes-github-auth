...
When a user tries to authenticate to the Kubernetes API, the Kubernetes apiserver
...
Kubernetes resources a user can access.

## How to use

First of all, you need to run the authenticator using the example [DaemonSet
manifest](manifests/github-auth.yaml). It is recommended to run the
...
```
kubectl create -f https://raw.githubusercontent.com/next-stack/kubernetes-github-auth/master/manifests/github-auth.yaml
```

Confirm that the authenticator is running:

```
kubectl get ds -l k8s-app=github-auth -n kube-system
```
...
```
kubectl create -f https://raw.githubusercontent.com/next-stack/kubernetes-github-auth/master/manifests/admin-cluster-role.yaml
```
